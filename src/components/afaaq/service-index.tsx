    ...
    <div className="mt-12 md:mt-14">
      {primaryServices.map((service, index) => (
        <article key={service.slug} id={service.slug} className="grid gap-5 border-t border-white/18 py-7 md:grid-cols-12 md:gap-8 md:py-9">
          <p className="font-technical m-0 text-[0.78rem] font-medium leading-5 text-white/60 md:col-span-1">{String(index + 1).padStart(2, "0")}</p>
          <div className="md:col-span-6">
            <h3 className="m-0 text-[clamp(1.75rem,3vw,2.8rem)] font-medium leading-[1.04] tracking-[-0.035em]">
              {service.title}
            </h3>
          </div>
          <div className="md:col-span-5">
            ...
            <Link
              ...
              className="group mt-4 inline-flex min-h-12 items-center gap-3 text-[0.95rem] font-medium text-white/90"
            >
