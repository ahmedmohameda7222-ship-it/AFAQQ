  ...
  return (
    <>
      <section className="pb-20 pt-14 md:pb-28 md:pt-20">
        <Container>
          <div className="grid gap-10 md:grid-cols-12 md:items-end">
            ...
            <p className="m-0 max-w-lg text-[1.05rem] leading-8 text-[var(--muted)] md:col-span-4">{service.intro}</p>
          </div>
        </Container>
      </section>
        ...
        <Container>
          <div className="grid gap-12 md:grid-cols-12">
            <div className="md:col-span-4">
              ...
              <h2 className="mt-6 text-[clamp(2.2rem,4.2vw,3.8rem)] font-medium leading-[1] tracking-[-0.047em]">
                Clear technical scope, from checks to final verification.
              </h2>
            </div>
            <div className="md:col-span-7 md:col-start-6">
              {service.capabilities.map((item, index) => (
                <div key={item} className="grid grid-cols-[3rem_1fr] gap-4 border-t border-white/18 py-5 last:border-b">
                  <span className="text-xs font-semibold tracking-[0.1em] text-white/40">{String(index + 1).padStart(2, "0")}</span>
                  <p className="m-0 text-[1.05rem] font-medium leading-7">{item}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 md:py-28">
          <Container>
            <div className="grid gap-10 md:grid-cols-12">
              ...
              <div className="grid gap-x-8 md:col-span-7 md:col-start-6 md:grid-cols-2">
                {service.methods.map((item) => (
                  <p key={item} className="m-0 border-t border-[var(--rule)] py-5 text-[1rem] font-medium leading-7">{item}</p>
                ))}
              </div>
            </div>
          </Container>
      ...
      {service.standards?.length ? (
        <section className="border-y border-[var(--rule)] py-16 md:py-20">
          <Container>
            <div className="grid gap-8 md:grid-cols-12 md:items-center">
              <div className="md:col-span-4"><SectionLabel>Relevant Standards & Protocols</SectionLabel></div>
              <div className="flex flex-wrap gap-x-7 gap-y-4 md:col-span-7 md:col-start-6">
                {service.standards.map((item) => (
                  <span key={item} className="text-[clamp(1.35rem,2.5vw,2rem)] font-medium tracking-[-0.03em]">{item}</span>
                ))}
              </div>
            </div>
          </Container>
          ...
          <Container>
            <div className="grid gap-10 md:grid-cols-12">
              <div className="md:col-span-4">
                ...
                <h2 className="mt-6 text-[clamp(2.2rem,4vw,3.6rem)] font-medium leading-[1] tracking-[-0.045em]">Tools for electrical testing.</h2>
              </div>
              <div className="md:col-span-7 md:col-start-6">
                {equipment.map((item) => (
                  <p key={item} className="m-0 border-t border-[var(--rule)] py-4 text-[0.98rem] font-medium last:border-b">{item}</p>
                ))}
              </div>
            </div>
          </Container>
          ...
          <Container>
            ...
            <div className="mt-8 grid gap-0">
              {relatedProjects.map((project) => (
                <ArrowLink key={project.slug} href={`/projects/${project.slug}`} className="w-full border-t border-[var(--rule)] py-5 last:border-b text-lg">
