...
assert(relationships.includes("localSrc"), "Relationship rail must support local relationship marks.");
assert(relationships.includes("google.com/s2/favicons"), "Relationship rail must retain favicon fallback for companies without verified local assets.");
assert(relationships.includes('EGEMAC: { domain: "egemac.com.eg" }'), "EGEMAC must use its official website icon fallback.");
assert(
  relationships.includes('"Arab Organization for Industrialization": { domain: "aoi.org.eg" }'),
  "Arab Organization for Industrialization must use its official website icon fallback.",
);
...
assert(relationships.includes("aria-hidden={isDuplicate}"), "Duplicate loop items must be hidden from assistive technology.");
...
assert(globals.includes("animation-play-state: paused"), "Relationship marquee must pause on hover/focus/manual control.");
